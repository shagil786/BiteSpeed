...
const App = () => {
  ...
  const [nodeSelected, setNodeSelected] = useState<any>(false)
  const [changeNode, setChangeNode] = useState<any>(null)
  const [errorMessage, setErrorMessage] = useState<any>(null) // custom error message notification
  const [messageColor, setMessageColor] = useState<any>(null) // custom color for error & success notification
  const [targetHandles, setTargetHandles] = useState<any>([]) // keep track of target handles when new edges are created between nodes

  // determine when a node is selected by user
  // console.log(nodes, reactFlowInstance);
  const update = useCallback((event: any, node: any) => {
  ...
  const saveFlow = () => {
    ...
    const totalNodes = (reactFlowInstance as  any)?.getNodes().length
